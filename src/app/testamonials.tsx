...
export default function Testamonials() {
  return (
    <section className="flex flex-col p-6" id="testimonials">
      <div className="flex flex-col items-center container px-4 md:px-6 max-w-[1100px] gap-y-16">
        <h2 className="text-3xl font-bold tracking-tighter text-[#006a4e] sm:text-4xl md:text-5xl lg:text-6xl/none text-center">
          CaredFor has helped many people
        </h2>

        <Carousel className="w-3/4 lg:w-1/2" opts={{ loop: true }} >
          <CarouselContent>
            {testamonials.map((testamonial, i) => (
              <CarouselItem key={i}>
                <div className="flex flex-col items-center space-y-4">
                  ...
                  <p className="text-gray-500 text-lg text-center bg-[#F1F8F5] p-6 rounded-2xl">
                    "{testamonial.message}"
                  </p>
                </div>
              </CarouselItem>
